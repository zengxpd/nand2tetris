...
class CodeWriter:
    ...
    def _write_comparison(self, jump_instruction, label_prefix):
        self.file.write(
            f"@SP\n"
            f"AM=M-1\n"
            f"D=M\n"
            f"A=A-1\n"
            f"D=M-D\n"
            f"@{self.output_file_name}.{label_prefix}.{self.label_counter}\n"
            f"D;{jump_instruction}\n"
            f"D=0\n"
            f"@{self.output_file_name}.{label_prefix}.{self.label_counter}.end\n"
            f"0;JMP\n"
            f"({self.output_file_name}.{label_prefix}.{self.label_counter})\n"
            f"D=-1\n"
            f"({self.output_file_name}.{label_prefix}.{self.label_counter}.end)\n"
    ...
    def write_arithmetic(self, command):
        ...
        if command == 'add':
            ...
            self._write_comparison('JEQ', 'eq')
        elif command == 'gt':
            self._write_comparison('JGT', 'gt')
        elif command == 'lt':
            self._write_comparison('JLT', 'lt')
        elif command == 'and':
            self.file.write('@SP\nAM=M-1\nD=M\nA=A-1\nM=D&M\n')
        elif command == 'or':
            self.file.write('@SP\nAM=M-1\nD=M\nA=A-1\nM=D|M\n')
